...
struct PrimusSwapchain{
  ...
  PrimusSwapchain(InstanceInfo &myInstance, VkDevice device, VkDevice display_device, VkSwapchainKHR backend, const VkSwapchainCreateInfoKHR *pCreateInfo, std::shared_ptr<CreateOtherDevice> &cod):
    ...
    if(getenv("PVK_SUPPRESS_SUBOPTIMAL")){
      suppress_suboptimal = true;
    }
...
VkResult VKAPI_CALL PrimusVK_AcquireNextImage2KHR(VkDevice device, const VkAcquireNextImageInfoKHR* pAcquireInfo, uint32_t* pImageIndex) {
  ...
  if(ch->suppress_suboptimal && res == VK_SUBOPTIMAL_KHR){
    res = VK_SUCCESS;
  }
...
void PrimusSwapchain::present(const QueueItem &workItem){
...

    {
      ...
      if(suppress_suboptimal && res == VK_SUBOPTIMAL_KHR){
	res = VK_SUCCESS;
      }
                                     ...
                                     uint32_t *pPropertyCount, VkExtensionProperties *pProperties)
{
  ...
  if(pLayerName == NULL || strcmp(pLayerName, "VK_LAYER_PRIMUS_PrimusVK"))
  {
    ...
    uint32_t allProps = 0;
    auto res = instance_dispatch[GetKey(physicalDevice)].EnumerateDeviceExtensionProperties(physicalDevice, pLayerName, &allProps, nullptr);
    extensions = std::make_unique<std::vector<VkExtensionProperties>>(allProps);
    ...
    res = instance_dispatch[GetKey(physicalDevice)].EnumerateDeviceExtensionProperties(physicalDevice, pLayerName, &allProps, extensions->data());
    if(res != VK_SUCCESS) {
      return res;
    }
    ...
    auto mProps = extensions->data();
    int outPos = 0;
    int inPos = 0;
    *pPropertyCount = allProps;
    for(int i = 0; i < allProps; i++) {
      if(std::string{"VK_NV_low_latency"} == mProps[i].extensionName ||
	 std::string{"VK_NV_low_latency2"} == mProps[i].extensionName) {
	if(pProperties)
	  std::copy(&mProps[inPos], &mProps[i], &pProperties[outPos]);
	(*pPropertyCount)--;
	outPos += i - inPos;
	inPos = i + 1;
      }
    ...
    if(pProperties)
      std::copy(&mProps[inPos], &mProps[allProps], &pProperties[outPos]);
    return res;
  }
